...
public class GameController {
	...
	public void auction(Property property) {
		...
		List<Player> players =  game.getPlayers();
		ArrayList<Player> auctionPlayers = new ArrayList<>();
		for(Player p: players){
			if (!p.isBroke()){
		...
		while (auctionPlayers.size() > 1){
			...
			Player currentBidder = auctionPlayers.remove(0);

				String bidSelection = gui.getUserSelection("does " + currentBidder.getName()+ " want to bid on " + property.getName(), "yes" , "no");
				if(bidSelection.equals("yes")) {
					biddingValue = gui.getUserInteger("How much do you bid? " + currentBidder.getName(), biddingValue+1, Integer.MAX_VALUE);
					auctionPlayers.add(currentBidder);
				} else {
					gui.showMessage(currentBidder.getName() + " has been removed from the auction.");
				}
		}

		if(auctionPlayers.size() == 1){
			Player winner =  auctionPlayers.get(0);
			property.setOwner(winner);
			gui.showMessage("Congrats to "+ winner.getName() +" You got the property :P");
			winner.payMoney(biddingValue);
		}
		else {
			gui.showMessage("No bidders");
		}
